...
public class AddAccountServiceImpl implements AddAccountService {
    ...
    public String add_1(String serviceName,String request) {
        ...
        String query2 = "SELECT COUNT(*) FROM test.tbl_account";
        String query3 = "INSERT into test.tbl_account (accnumber,pass,acctype,code) values(?,?,?,?)";
        String query4 = "SELECT * FROM test.tbl_account where def=?";
        String query5="select max(code) from test.tbl_account where acctype=?";
        ...
        try {
            ...
            int code=0;
            if(rs.next()){
            code=Integer.parseInt(rs.getString(1))+1;
            }
